...
export default async function RecentPayments() {
	...
	return (
		<div className="w-full flex flex-col gap-6">
			<h1 className={`${lusitana.className} text-2xl font-bold text-purple-700 ml-2`}>Recent Payments</h1>
			<div className="w-full rounded-lg bg-white shadow-md p-4">
				<table className="w-full">
					...
					<tr className="text-left text-gray-600 text-sm font-semibold border-b border-gray-200">
						...
						<th className="p-3 whitespace-nowrap overflow-hidden">Date</th>
					</tr>
					</thead>
					<tbody>
					{latestPayments.map((payment, index) => (
						...
						>
							<td className="p-3 text-gray-700">{trimName(payment.fullname)}</td>
							<td className="p-3 text-gray-700">{`$${payment.amount.toFixed(2)}`}</td>
							<td className="p-3 text-gray-500">{formatDate(payment.createdAt)}</td>
						</tr>
					))}
					</tbody>
				</table>
