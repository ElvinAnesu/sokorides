...
export default async function RecentPayments() {
	...
	return (
		<div className="w-full flex flex-col gap-2 mt-3">
			<h1 className={`${lusitana.className} text-xl font-bold text-purple-700 ml-2`}>Recent Payments</h1>
			<div className="w-full rounded-lg bg-white shadow-md p-2">
				<table className="w-full">
					...
					{latestPayments.map((payment, index) => (
						...
						>
							<td className="p-3 text-gray-500">{trimName(payment.fullname)}</td>
							<td className="p-3 text-gray-500">{`$${payment.amount.toFixed(2)}`}</td>
