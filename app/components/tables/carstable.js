...
export default function CarsTable() {
	...
	const getProducts = async () => {
		...
		const response = await fetch(
			`/api/products/?pageSize${PAGE_SIZE}&page=${page}&searchParams=${searchParams}`,
			{
				method: "GET",
	...
	return (
		<div className="flex flex-col w-full h-full gap-2">
			<div className="flex w-full items-center justify-end">
				<button
					className="bg-purple-900 rounded text-white text-sm px-2 py-2 flex items-center gap-1"
					onClick={() => router.push("/dashboard/cars/createnew")}
				>
					<PlusIcon /> Add New
				</button>
			</div>
						...
						<tbody>
							...
							{totalProducts === 0 ? (
								...
								products.map((car, index) => (
									<tr className="border-b border-gray-500 text-sm" key={index}>
										...
										<td className="px-2 flex items-center gap-4">
											<button
												onClick={() =>
											...
											<button onClick={() => deleteProduct(car._id)}>
												<TrashIcon />
											</button>
										</td>
									</tr>
