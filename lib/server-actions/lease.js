...
export async function addClientDetails(params, formData) {
	...
	if (monthlyPayments > totalPrice) {
		return {
			errors: {
				monthlyPayments:
					"Monthly payments cannot be greater than the total price.",
			},
		};
	}
	if (downPayment > totalPrice) {
		return {
			errors: {
				downPayment: "Down payment cannot be greater than the total price.",
			},
		};
	} 
		if (monthlyPayments > totalPrice - downPayment) {
			return {
				errors: {
					downPayment: "Down payment cannot be greater than the balance.",
				},
			};
		} 
...
export async function deleteLease(_id) {
	try {
		...
		console.log("deleted")
		 ...
		 throw new Error(
				`Failed to delete lease with ID : ${error.message}`
			);
	} 
	revalidatePath("/dashboard/rent-to-buy/leasedcars");
	redirect("/dashboard/rent-to-buy/leasedcars");
} 
